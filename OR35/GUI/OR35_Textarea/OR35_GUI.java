...
public class OR35_GUI {
  ...
  final static String[] wait_text = { "Waiting for a number", "Waiting for a string", "Waiting for input"};
  ...
  static JRadioButton option_factor, option_char, option_both;
  ...
  public static void main(String[] args) {
    ...
    option_both = new JRadioButton("Both");
    option_both.addItemListener (new BothRadioButtonListener ()); 
    
    // Associate the radio buttons with the same group
    option_group = new ButtonGroup();
    option_group.add(option_factor);
    option_group.add(option_char);
    option_group.add(option_both);
  ...
  private static class BothRadioButtonListener implements ItemListener {
    @Override
    public void itemStateChanged(ItemEvent e) {
      check_type = 2;
      text_area.setText (wait_text[check_type]);
